...
def supermarkets() -> list[SupermarketInfo]:
    return (
        catalog_service.get_supermarkets()
    )  # Supermercados disponibles y estado de su catálogo
...
@router.api_route(
    "/supermarkets", methods=["QUERY"], response_model=list[SupermarketInfo]
)
# Filtros opcionales en el body (Nuevo metodo HTTP) y la respuesta es una lista de SupermarketInfo
def query_supermarkets(
    filters: SupermarketFilter | None = None,
) -> list[SupermarketInfo]:
    return catalog_service.get_supermarkets(
        name=filters.name
        if filters
        else None,  # Si filters es None, se pasa None, si no, se pasa filters.name
    )


@router.get("/categories", response_model=list[str])
def categories(
    supermarket: str | None = Query(None, description="Filter by supermarket"),
) -> list[str]:
    if supermarket and supermarket not in SUPERMARKETS:
        raise HTTPException(404, f"Supermarket not found: {supermarket}")
    return catalog_service.get_categories(supermarket)
...
def products(
    supermarket: str | None = Query(
        None, description="mercadona, consum, masymas or lidl"
    ),
    q: str | None = Query(
        None,
    ...
    )  # Lista los productos guardados, con filtros y paginación
